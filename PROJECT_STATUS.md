...
Добавлен скрипт управления webhook `deploy/scripts/manage-webhook.sh`, обновлены `apps/web/package.json`, `compose.production.yaml`, `deploy/deploy.sh`, `PROJECT_STATUS.md` и `docs/DEPLOYMENT.md`.
 
## Следующий конкретный шаг
 
Провести пользовательское тестирование в мессенджере MAX:
1. Открыть диалог с ботом `@se14396800_bot` и отправить `/start`.
2. Нажать «Открыть профиль» (открывает `https://158-160-237-183.sslip.io`) и сохранить профиль жильца.
3. Отправить триггерное сообщение в чат `-79181109403700` и проверить получение личного алерта.
4. Запросить сводку за сегодня / 7 дней / 30 дней.
