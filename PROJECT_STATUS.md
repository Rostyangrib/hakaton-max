...
Реализованы три согласованных ключевых улучшения системы:
1. **Дружелюбный Pre-check в Mini App**:
   - `GET /api/profile` возвращает статус `isMember`, название чата `homeChatTitle` и ссылку на чат `homeChatUrl` (с поддержкой `chatId` query-параметра).
   - В UI отображается предупреждающая карточка с золотисто-желтым акцентом `#F3AE0D` и кнопкой вступления в чат. Кнопка сохранения блокируется с понятной подсказкой, если пользователь не состоит в чате.
   - Ошибки сохранения локализованы на русский язык (403 Forbidden с пояснением).
2. **Отзыв доступа при выходе из чата (JIT Check & Webhook)**:
   - Just-in-time проверка членства в `SummaryService.generate` с 30-секундным in-memory кэшем.
   - Сброс `membership_verified_at` в `null` и генерация вежливого русскоязычного уведомления в ЛС бота при попытке получить выжимку после выхода из чата.
   - Обработка webhook-событий `user_removed` (отзыв доступа, сброс кэша, уведомление жильца), `chat_title_changed`, `bot_added` и `bot_removed`.
...
4. **Качество и тесты**:
   - Пройдены все 107 тестов монорепозитория (`pnpm test`), `pnpm typecheck` без ошибок, `vite build` без ошибок.
