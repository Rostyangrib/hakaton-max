...
## Актуальные изменения и доработки

Реализованы задачи по доработке мультичатовости, алертов и Mini App:
1. **Персональные алерты с указанием названия домового чата**:
   - Формат персонального алерта обновлен: при наличии названия чата формируется текст: `🔔 В домовом чате «{homeTitle}» упомянули: {trigger.label}.`
   - `MessageRepository` дополнен методом `getHomeTitle(homeId: string)`, извлекающим название дома из таблицы `homes`.
   - В `MessagePipeline.createAlerts` извлекается название дома и передается в `sendPrivateAlert`.
   - Усилен парсинг `chat_id` в `parseIncomingMessage` и `handleRemoved` для строковых/отрицательных ID.
   - Выявлена и задокументирована причина отсутствия событий из чата 2: бот не был назначен администратором с правом `read_all_messages` в MAX.
2. **Отключение 15-минутного антифлуда**:
   - `ALERT_ANTIFLOOD_MINUTES` в `packages/config` обновлен до `z.coerce.number().int().min(0).default(0)`.
   - В конфигурациях `.env.*` и `compose.*` значение по умолчанию установлено в `0`.
   - `PostgresMessageRepository.reserveDelivery` при `antifloodMinutes <= 0` минует проверку предыдущих интервалов, обеспечивая моментальную доставку алертов.
3. **Обновление интерфейса Mini App и формулировок**:
   - Убраны лишние эмодзи (`🏠` в табах домов, `🔄` в кнопке «Проверить статус»).
   - Табы домов в Mini App теперь отображают **только** чаты, где пользователь состоит (`isMember === true`), без плашек «Не в чате».
   - Табы отображают активный дом жильца (`[ Тест 2 ]`), подтверждая выбор при наличии доступных домов (`memberHomes.length > 0`).
   - При отсутствии участия в домовых чатах (`memberHomes.length === 0`) форма профиля, поля ввода и кнопка сохранения скрываются, отображается только информационная карточка `.not-member-card`.
   - API `GET /api/profile` возвращает явный `chatId` активного дома и автоматически выбирает первый чат участия жильца.
   - Все упоминания «QuietChat» / «quiet chat» в приветствиях, системных сообщениях и Mini App заменены на русскоязычное «Тихий Чат».
4. **Качество и тесты**:
   - Пройдено 128 тестов во всех воркспейсах монорепозитория (`pnpm test`).
   - `pnpm typecheck` выполняется без ошибок (code 0).
   - `pnpm --filter @quiet-chat/web build` завершается успешно.
