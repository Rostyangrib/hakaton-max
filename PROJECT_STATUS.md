...
Реализован `SummaryCallbackHandler` (`apps/worker/src/summary-callback-handler.ts` и `apps/worker/src/summary-callback-handler.test.ts`), внедрено монотонное версионирование запросов для защиты от race conditions и дублирования сообщений при повторных кликах, обновлен `apps/worker/src/main.ts`, устранены «стакание» и зависание сообщений о генерации сводки, дополнен `CHANGELOG.md`.
...
3. Проверить быстрые повторные клики по кнопкам периодов: в чате должен отображаться только один актуальный статус и одна итоговая сводка.
