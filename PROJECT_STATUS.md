...
Реализован `SummaryCallbackHandler` (`apps/worker/src/summary-callback-handler.ts` и `apps/worker/src/summary-callback-handler.test.ts`), обновлен `apps/worker/src/main.ts`, устранены «стакание» и зависание сообщений о генерации сводки, добавлен `CHANGELOG.md`.
 
## Следующий конкретный шаг
 
Провести пользовательское тестирование в мессенджере MAX:
1. Запросить сводку за сегодня / 7 дней / 30 дней в диалоге с ботом `@se14396800_bot`.
2. Убедиться, что статусное сообщение появляется внизу диалога и на месте превращается в готовую сводку без накопления («стакания») старых статусных сообщений.
