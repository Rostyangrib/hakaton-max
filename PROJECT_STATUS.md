...
Реализованы три согласованных ключевых улучшения системы с устранением 8 критических дефектов:
1. **Дружелюбный Pre-check в Mini App**:
   - `GET /api/profile` возвращает статус `isMember`, название чата `homeChatTitle` и ссылку на чат `homeChatUrl` (с поддержкой валидированного числового `chatId` query-параметра, 400 при некорректном формате).
   - Сохранение профиля: `readProfile` больше не возвращает `null` при `membershipVerifiedAt = null`, сохраняя введенные данные (квартиры, автомобили) при временном выходе из чата.
   - В UI отображается предупреждающая карточка с золотисто-желтым акцентом `#F3AE0D`, кнопкой вступления в чат и кнопкой «🔄 Проверить статус». Кнопка сохранения блокируется с понятной подсказкой, если пользователь не состоит в чате.
   - Автоматическая проверка членства при возврате в Mini App (`focus` и `visibilitychange`) с инвалидацией негативного кэша (`?refresh=1`).
   - Ошибки сохранения локализованы на русский язык (403 Forbidden с понятным пояснением).
2. **Отзыв доступа при выходе из чата (JIT Check & Webhook)**:
   - Just-in-time проверка членства в `SummaryService.generate` с кэшем (30с для участников, 5с для неучастников).
   - Сброс `membership_verified_at` в `null` и генерация вежливого русскоязычного уведомления в ЛС бота с кнопкой возврата в чат при попытке получить выжимку после выхода.
   - Разблокировка жильцов: при возвращении в чат репозиторий находит профиль, а JIT-проверка автоматически восстанавливает `membershipVerifiedAt`.
   - Защита от утечки персональных алертов: `MessagePipeline` проверяет актуальное членство в чате перед доставкой алертов по авто и квартирам.
   - Обработка webhook-событий `user_removed` (отзыв доступа, сброс кэша, уведомление жильца), `user_added` (восстановление доступа, приветствие), `chat_title_changed`, `bot_added` и `bot_removed`.
3. **Мультичатовость (Multichat)**:
   - `MessagePipeline` маршрутизирует сообщения любого домового чата к его `homeId` через `ensureHome(BigInt(message.chatId))`.
   - Полная изоляция истории, алертов и сводок по домам.
   - Поддержка нескольких домов для одного жильца в Telegram/MAX боте: интерактивные inline-кнопки выбора дома при запросе сводки и кнопка «🏠 Сменить дом».
   - Заголовок сводки явно содержит название дома (`homeTitle`).
4. **Качество и тесты**:
   - Пройдены все 115 тестов монорепозитория (`pnpm test`), `pnpm typecheck` без ошибок, `vite build` без ошибок.
...
3. Провести smoke-тестирование (health/ready, проверка статусов контейнеров и логов).
