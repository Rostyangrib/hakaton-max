...
Реализованы три согласованных задачи пользователя:
1. **Устранение предзаполнения полей литералом "undefined"**:
   - В `apps/web/src/App.tsx` в функции `fromProfile` внедрена безопасная проверка `!= null`, гарантирующая, что поля `apartment`, `entrance`, `floor` всегда остаются пустыми строками `''` при отсутствии данных.
   - В `apps/api/src/app.ts` эндпоинт `GET /api/profile` изолирует незаполненный профиль (`profile: null`), не передавая неопределенные свойства.
   - Созданы модульные тесты в `apps/web/src/profile-form.test.ts`.
2. **Гарантированная доставка push-уведомлений в мобильное приложение MAX**:
   - Во все методы отправки личных сообщений (`bot.api.sendMessageToUser`) в `apps/worker` добавлен обязательный флаг `{ notify: true, format: 'markdown' }`.
   - В вызовах `user_added` и `user_removed` добавлено структурированное логирование через `console.warn` вместо пустого перехвата ошибок.
   - Обернут клиент `bot.api` для `MessagePipeline`, гарантирующий мобильные push-уведомления для всех алертов.
3. **Мультичатовый переключатель домов в Mini App**:
   - Бэкенд API `GET /api/profile` возвращает список доступных домов пользователя `availableHomes` со статусом `isMember` и ссылками на чат.
   - Добавлен метод `getActiveHomes` в `persistence.ts` и контракт `ProfileStore`.
   - Поддержана передача `chatId` в теле запроса и query-параметрах `PUT /api/profile` для раздельного сохранения профилей по домам.
   - В Mini App реализован интерактивный переключатель домов (`.home-selector` и `.home-tabs`) в секции «Адрес» с переключением формы и загрузкой данных конкретного дома.
   - Добавлена поддержка переключения домов в демо-режиме (`?demo=1`).
4. **Качество и тесты**:
   - Пройдено 123 теста во всех воркспейсах монорепозитория (`pnpm test`).
