...
3. **Обновление интерфейса Mini App и формулировок**:
   ...
   - Табы показываются только при наличии 2 и более чатов участия.
   - Если пользователь не состоит ни в одном домовом чате, табы скрываются и выводится понятная карточка-предупреждение.
   - API `GET /api/profile` автоматически выбирает первый чат участия пользователя при отсутствии явно указанного.
   - Все упоминания «QuietChat» / «quiet chat» в приветствиях, системных сообщениях и Mini App заменены на русскоязычное «Тихий Чат».
4. **Качество и тесты**:
   - Пройдено 127 тестов во всех воркспейсах монорепозитория (`pnpm test`).
