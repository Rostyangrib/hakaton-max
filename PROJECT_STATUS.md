...
Выполнен полный редизайн веб-приложения Mini App (`apps/web`):
- Реализована шапка с контурной SVG-иллюстрацией домов и горящими желтыми окнами (`#F3AE0D`).
...
- Обновлена карточка пользователя («Важное — только для вас») с круглым аватаром жильца.
- Секция 01 («Адрес» с обязательными полями: Квартира, двухколоночная сетка Подъезд / Этаж) с акцентным номером «01».
- Секция 02 («Автомобиль»: Госномер, Описание со встроенным счетчиком символов в углу) с серым номером «02».
- Блок «Личные уведомления» с иконкой колокольчика `#188DB5` и переключателем Toggle Switch.
- Статусная плашка «Профиль заполнен» в мятно-зеленой гамме `#E0F4F1`.
- Широкая CTA-кнопка «Сохранить изменения» (`#188DB5`).
- Подвал с центрированным замком и текстом «Данные видит только бот QuietChat».
- Успешно пройдены все тесты раскладки и адаптивности `apps/web/src/styles.test.ts`, `pnpm test`, `pnpm typecheck` и production-сборка Vite.
