...
export function detectMessageTriggers(text: string): MessageTrigger[] {
  ...
  const apartments = collectNumbers(normalized, [
    /(?:кв(?:артира|артире|артиру|артиры|\.)?|кварт(?:ира|ире|иру|иры|а))\s*(?:№|no\.?)?\s*(\d{1,4})(?!\d)/gu,
    /(?<!\d)(\d{1,4})\s*(?:-?(?:я|ая|ой|ую))?\s*(?:кв(?:артира|артире|артиру|артиры|\.)?|кварт(?:ира|ире|иру|иры|а))(?![\p{L}])/gu,
  ]);
  const entrances = collectNumbers(normalized, [
    /(?:подъезд(?:е|а|у|ом)?|под\.)\s*(?:№|no\.?)?\s*(\d{1,3})(?!\d)/gu,
    /(?<!\d)(\d{1,3})\s*(?:-?(?:й|ый|ой|м|го))?\s*(?:подъезд(?:е|а|у|ом)?|под\.)(?![\p{L}])/gu,
  ]);
